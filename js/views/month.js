...
import {
  ...
  generatePlan, currentDebtTotal, buildMonthSummary,
} from '../calc.js';
...
// ── Resumo histórico ─────────────────────────────────────────────
function renderOverview(d, r) {
  const el = document.getElementById('month-overview');
  if (!el) return;
  if (!d) { el.hidden = true; return; }

  const allSnaps  = snapshots.getAll();
  const dsMap     = debtStateStore.init(d.debts || []);
  const fixasSum  = (d.fixas  || []).reduce((s,f) => s + Number(f.value),       0);
  const debtsSum  = (d.debts  || []).reduce((s,db) => s + Number(db.installment), 0);
  const cost      = fixasSum + debtsSum;
  const metaFull  = cost * (d.metaMonths || 6);
  const phrases   = buildMonthSummary(allSnaps, r.saldo, d.debts || [], dsMap, metaFull);

  if (!phrases.length) { el.hidden = true; return; }

  el.innerHTML = `
    <div class="sh">Evolução financeira</div>
    <div class="month-overview">
      ${phrases.map(p => `<div class="overview-phrase">${p}</div>`).join('')}
    </div>`;
  el.hidden = false;
}
